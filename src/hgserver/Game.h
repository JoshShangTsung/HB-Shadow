...
public:
	...
	bool bPlantSeedBag(int dX, int dY, int iItemEffectValue1, int iItemEffectValue2, int iClientH);
	...
	void _SendMapStatus(CClient &client);
	void MapStatusHandler(CClient &client, int iMode, char * pMapName);
	void SelectCrusadeDutyHandler(CClient &client, int iDuty);
	void RequestSummonWarUnitHandler(CClient &client, int dX, int dY, char cType, char cNum, char cMode);
	...
	void RequestSetGuildConstructLocHandler(CClient &client, int dX, int dY, int iGuildGUID, char * pMapName);
	...
	void CheckCommanderConstructionPoint(CClient &client);
	...
	bool __bSetConstructionKit(int dX, int dY, int iType, int iTimeCost, CClient &client);
	...
	void RequestChangePlayMode(CClient &client);
	void GetHeroMantleHandler(int iClientH, int iItemID, char * pString);
	void AdminOrder_Weather(CClient &client, char * pData, uint32_t dwMsgSize);
	...
	void AdminOrder_SetStatus(CClient &client, char *pData, uint32_t dwMsgSize);
	...
	void AdminOrder_SummonStorm(CClient &client, char* pData, uint32_t dwMsgSize);
	...
	void AdminOrder_SetObserverMode(CClient &client);
	void AdminOrder_EnableAdminCommand(CClient &client, char *pData, uint32_t dwMsgSize);
	void AdminOrder_CreateItem(CClient &client, char * pData, uint32_t dwMsgSize);
	...
	void RequestHelpHandler(CClient &client);
	void CheckConnectionHandler(CClient &client, char *pData);
	...
	void CancelQuestHandler(CClient &client);
	void ActivateSpecialAbilityHandler(CClient &client);
	void EnergySphereProcessor(bool bIsAdminCreate = false, int iClientH = 0);
	void JoinPartyHandler(CClient &client, int iV1, char *pMemberName);
	void CreateNewPartyHandler(CClient &client);
	...
	void _TamingHandler(int iClientH, int iSkillNum, MapPtr map, int dX, int dY);
	...
	void FishProcessor();
	int iCheckFish(int iClientH, MapPtr map, short dX, short dY);
