	...
	if (iPreLoc == 0) {
		...
		if ((sAppr4 & 0x00F0) != 0) {
			...
			switch (m_pData[dX][dY].m_iEffectType) {
				case 1: m_pData[dX][dY].m_iEffectTotalFrame = 13;
					break;
				case 2: m_pData[dX][dY].m_iEffectTotalFrame = 11;
					break;
			}
		} else {
			m_pData[dX][dY].m_iEffectType = 0;
...
int CMapData::iObjectFrameCounter(char * cPlayerName, short sViewPointX, short sViewPointY) {
	...
	int cTotalFrame;
	int cFrameMoveDots;
	static uint32_t S_dwUpdateTime = timeGetTime();
	int sWeaponType;
	int sCenterX;
	int sCenterY;
	int sDist; //, sAbsX, sAbsY;
								...
								if ((m_pData[dX][dY].m_cOwnerFrame == 1) || (m_pData[dX][dY].m_cOwnerFrame == 5)) {
									...
									if (((m_pData[dX][dY].m_sAppr4 & 0x000F) != 0) && ((m_pData[dX][dY].m_iStatus & 0x10) == 0)) {
										...
										switch (m_pData[dX][dY].m_cDir) {
											case 1: dy = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 2: dy = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												dx = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 3: dx = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 4: dx = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												dy = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 5: dy = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 6: dy = -cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												dx = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 7: dx = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
											case 8: dx = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												dy = cFrameMoveDots * (cTotalFrame - m_pData[dX][dY].m_cOwnerFrame);
												break;
										}
